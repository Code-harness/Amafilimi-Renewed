...
@section('title', 'Amafilimi | Upload Movie')
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-4xl font-black text-slate-900 tracking-tight">Upload Movie</h1>
                <span
                    class="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-emerald-100 flex items-center gap-1.5">
                    <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
                    New Release
                </span>
            </div>
            <p class="text-slate-500 mt-2 font-medium text-lg">
                Add a new movie to the Amafilimi streaming library.
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div class="xl:col-span-2 space-y-8">
                ...
                <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                    <div class="p-8 border-b border-slate-50">
                        <h3 class="font-black text-slate-900 text-xl tracking-tight">Movie Information</h3>
                        <p class="text-sm text-slate-500 mt-1">Core details about the movie.</p>
                    </div>

                    <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            <label class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-3">Movie Title *</label>
                        ...
                        <div>
                            <label class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-3">Duration (Minutes) *</label>
                            <input type="number" name="duration" value="{{ old('duration') }}"
                                class="w-full rounded-2xl border border-slate-200 px-5 py-4 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
                                placeholder="120">
                            @error('duration') <p class="text-rose-500 text-xs mt-2 font-bold">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" rows="6"
                                class="w-full rounded-2xl border border-slate-200 px-5 py-4 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
                                placeholder="Write a compelling movie description...">{{ old('description') }}</textarea>
                ...
                <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                    <div class="p-8 border-b border-slate-50">
                        ...
                        <p class="text-sm text-slate-500 mt-1">Thumbnail, poster and trailer assets.</p>
                    </div>
                ...
                <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                    <div class="p-8 border-b border-slate-50">
                        <h3 class="font-black text-slate-900 text-xl tracking-tight">Playback Source</h3>
                        <p class="text-sm text-slate-500 mt-1">Streaming source and embed information.</p>
                    </div>

                    <div class="p-8 grid grid-cols-1 gap-6">
                        <div>
                            <label class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-3">Embed URL *</label>
                            <input type="url" name="embed_url" value="{{ old('embed_url') }}"
                                class="w-full rounded-2xl border border-slate-200 px-5 py-4 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
                                placeholder="Bunny.net embed URL">
                            @error('embed_url') <p class="text-rose-500 text-xs mt-2 font-bold">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-8">
                        ...
                        <div>
                            ...
                            <label class="flex items-center justify-between bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 cursor-pointer">
                                ...
                                <input type="checkbox" name="is_featured" value="1"
                                    class="w-5 h-5 rounded border-slate-600 text-emerald-500 focus:ring-emerald-500"
                ...
                <div class="bg-emerald-500 rounded-[2.5rem] p-8 text-white">
                    <p class="text-xs font-black uppercase tracking-widest opacity-80 mb-2">Ready to Publish</p>
                    <p class="text-3xl font-black mb-4">New Movie</p>
                    <p class="text-sm text-emerald-50/90 mb-6 leading-relaxed">
                        Save this movie into your streaming catalog and control how viewers access it.
                    </p>
